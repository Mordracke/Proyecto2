...
const int tamano = 10; // Tamaño del tablero
...
bool juegoTerminado(Jugador& oponente) {
    for (int i = 0; i < tamano; ++i) {
        for (int j = 0; j < tamano; ++j) {
            if (oponente.tablero[i][j] == 'B') {
                return false; // Todavía quedan barcos en el tablero del oponente
            }
        }
    }
...
void interfaz(){
    ...
    do{
        ...
        case 1:
            ...
            int otroJugador; // Declarar la variable otroJugador dentro del case 1
